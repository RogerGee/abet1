...
// create a hash of 'level' and 'id' to give the object a unique; id may
// come from the object if not specified; 'leve' is a string literal that
// remains constant for each level in the nav tree
function create_unique_id($obj,$level,$id = null) {
    if (is_null($level))
        throw new Exception("create_unique_id() expects non-null 'level'");
    if (is_null($id)) {
        if (!isset($obj->id))
            throw new Exception("create_unique_id() expects 'id' on object");
        $id = $obj->id;
    }
    $obj->identity = hash('md5',"$level$id",false);
}
...
function make_program_node($row,$parent) {
    ...
    create_unique_id($program,'program',$row['program.id']);
    ...
    if ($isAdmin) {
        ...
        create_unique_id($editProgram,'edit-program');
...
function make_criterion_node($row,$program,$programId) {
    ...
    create_unique_id($criterion,'criterion',$row['abet_criterion.id']);
    ...
    if ($isAdmin) {
        ...
        create_unique_id($createAssessment,'create-assessment');
...
function make_characteristic_node($row,$criterion) {
    ...
    create_unique_id($characteristic,'characteristic',$row['abet_characteristic.id']);
...
function make_assessment_node($row,$parent) {
    ...
    create_unique_id($assessment,'assessment',$row['abet_assessment.id']);
...
for ($i = 1;$i <= $query->get_number_of_rows();$i++) {
    ...
    if (!is_null($row['assessment_worksheet.id'])) {
        ...
        create_unique_id($content,'worksheet');
        ...
        if (!is_null($row['course_number']) || !is_null($row['activity'])) {
            ...
            create_unique_id($division,'worksheet-division',$row['assessment_worksheet.id']);
        }
        else
        ...
        if (!is_null($row['rubric.id'])) {
            ...
            create_unique_id($rubric,'rubric',$row['rubric.id']);
    ...
    if (!is_null($row['general_content.id'])) {
        ...
        create_unique_id($content,'general-content');
...
if ($isAdmin) {
    ...
    create_unique_id($editCharacteristics,'admin-tool',4);

    $editCourse = new stdClass;
    $editCourse->label = "Courses...";
    $editCourse->type = "editCourses";
    $editCourse->id = null;
    create_unique_id($editCourse,'admin-tool',5);
    ...
    create_unique_id($adminTools,'top-level',2);
